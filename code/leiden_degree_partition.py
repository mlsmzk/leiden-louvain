...
def degree_partition(G):
    ...
    Returns:
        dict: A partition of the graph where nodes with the same degree are in the same community.
    """
    partition = {}
    degree_to_community = {}
    for node in G.nodes():
        degree = G.degree(node)
        if degree not in degree_to_community:
            degree_to_community[degree] = len(degree_to_community)
        partition[node] = degree_to_community[degree]
    return partition
...
def first_phase(G, total_edges):
    """
    First phase of the Leiden algorithm with degree-based partitioning.
    ...
    """
    # Initialize the partition using degree-based partitioning
    partition = degree_partition(G)
...
def leiden_algorithm(G):
    ...
    while True:
        partition = first_phase(G, total_edges)
